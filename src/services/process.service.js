...
export async function getWorkOrderBillRegisterData(req, res) {
  ...
  try {
    const sql = `
      SELECT
          C.COMPCODE,
           SUM(B.WOQTY * B.BILLRATE) AS TOTALAMOUNT
...
SELECT  BB.WONO,BB.WODATE,A.WORKBILLNO,G.WODESC,I.ITEMNAME,A.SUPPLIER SUPPLIER1,B.WOQTY,B.BILLRATE,(B.WOQTY * B.BILLRATE) AS NETAMOUNT
FROM GTWOBILLENTRY A
JOIN GTWOBILLENTRYDET B ON A.GTWOBILLENTRYID=B.GTWOBILLENTRYID
