...
export async function getTopItems(req, res) {
    ...
    try {
        ...
        const sql =
            `
          SELECT NVL(A.BGF, 'NA') AS BGF,A.PAYCAT,COUNT(*) CNT FROM MISTABLE A
WHERE A.COMPCODE = '${filterBuyer}' AND A.DOJ <= (
...
GROUP BY NVL(A.BGF, 'NA'),A.PAYCAT
ORDER BY 2 DESC,1
     `
...
WHEN X.AGE BETWEEN 2 AND 3 THEN '2 - 3'
WHEN X.AGE BETWEEN 4 AND 5 THEN '3 - 5'
WHEN X.AGE BETWEEN 6 AND 10 THEN '6 - 10'
WHEN X.AGE >= 11 THEN '11 +'  END SLAP, x.idcard,x.DOJ,X.AGE FROM (
SELECT FLOOR(MONTHS_BETWEEN(TRUNC(SYSDATE),A.DOJ)/12) AGE,a.idcard,A.DOJ FROM MISTABLE A 
JOIN HREMPLOYDETAILS CC ON A.IDCARD = cc.IDCARD
JOIN HREMPLOYMAST BB ON BB.HREMPLOYMASTID = CC.HREMPLOYMASTID
