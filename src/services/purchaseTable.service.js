...
export async function getGeneralTable(req, res) {
  ...
  try {
    ...
    const sql = `
SELECT D.FINYR FINYEAR,C.COMPCODE,A.DOCDATE,A.DOCID,G.ITEMGRPNAME,I.ITEMNAME,A.SUPPLIER,B.POQTY-B.CANQTY POQTY,U.UNITNAME,B.PORATE,(B.POQTY-B.CANQTY)*B.PORATE AMOUNT FROM GTGENPO A
...
export async function getDyedYarnTable(req, res) {
  ...
  try {
    ...
    const sql = `
SELECT A.FINYR,A.COMPCODE,A.DOCDATE,A.DOCID,A.YARN,A.SUPPLIER,A.ORDERNO,A.COLORNAME,(A.POQTY - A.CANQTY) POQTY,A.UNITNAME,A.PRICE PRICE, (A.POQTY - A.CANQTY) * A.PRICE  VAL
FROM DYARNPURREG A
where A.FINYR = '${selectedYear}' AND A.COMPCODE = '${companyName}'
ORDER BY A.FINYR,A.COMPCODE
     `;
