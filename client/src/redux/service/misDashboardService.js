...
const MisDashboard = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // In misDashboardService.js — fix the method
    executeMISHR: builder.mutation({
      query: () => ({
        // no params needed
        url: MIS_DASHBOARD + "/execute-mishr",
        method: "PUT", // ← POST/PUT, not GET
        headers: {
          "Content-type": "application/json; charset=UTF-8",
        },
      }),
