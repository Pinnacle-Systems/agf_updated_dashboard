...
const AttriDetails = ({
  ...
  const [selectedGender, setSelectedGender] = useState(selectedGender1);
  ...
  console.log(salaryDet, "salaryDet1");
 ...
 const downloadExcel = async () => {
    ...
    /* =======================
       1️⃣ TITLE ROW
    ======================= */
    worksheet.addRow([excelTitle || "Attrition Report"]);
    ...
    headerRow.eachCell((cell) => {
      ...
      cell.fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: "FFD9D9D9" }, // gray header
      };
      cell.border = {
        top: { style: "thin" },
        bottom: { style: "thin" },
    ...
    filteredData.forEach((row) => {
      worksheet.addRow([
        ...
        row.DEPARTMENT,
        // row.COMPCODE,
        row.DOL ? new Date(row.DOL) : "N/A", // keep as Date
        row.REASON || "N/A",
      ]);
    });

    /* =======================
       5️⃣ DATA FORMATTING & ALIGNMENT
    ======================= */
    ...
    saveAs(
      new Blob([buffer]), `${excelTitle}.xlsx` ||
          ...
          >
            <table className="w-full border-collapse border border-gray-300 text-[11px]">
              <thead className="bg-gray-100 text-gray-800 sticky top-0 tracking-wider">
                <tr>
                  <th className="border pt-2 pb-2 text-left">S.No</th>
                  <th className="border pt-2 pb-2 text-left">IDCard</th>
                  <th className="border p-2 text-left">Name</th>
                  <th className="border  pt-2 pb-2 text-left">Gender</th>
                  <th className="border pt-2 pb-2 text-left">Department</th>
                  <th className="border p-2 text-left">DOL</th>
              ...
              <tbody>
                {currentRecords.slice(0, 17).map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      <td className="border p-1 text-[10px] w-[15px]">
                        {serialNo}
                      </td>
                      ...
                      <td
                        className="border p-1 text-[10px] w-[10px] whitespace-nowrap overflow-hidden text-ellipsis"
                        style={{ maxWidth: "10px" }}
                      >
                        {row.GENDER}
                      </td>
                      <td
                        className="border p-1 text-[10px] w-[80px] whitespace-nowrap overflow-hidden text-ellipsis "
                        style={{ maxWidth: "80px" }}
                      >
                        {row.DEPARTMENT}
                      </td>
                      <td
                        className="border p-1 text-[10px] w-[50px] whitespace-nowrap overflow-hidden text-ellipsis "
                        style={{ maxWidth: "50px" }}
                      >
                        {row.DOL
                      ...
                      <td
                        className="border p-1 text-[10px] w-[100px]  text-red-500 font-semibold "
                        style={{ maxWidth: "100px" }}
                      >
                        {row.REASON || "N/A"}
                      </td>
            ...
            <table className="w-full border-collapse border border-gray-300 text-[11px]">
              ...
              <tbody>
                {currentRecords.slice(17, 34).map((row, index) => {
                  const globalIndex = 117 + index; // 17–33
                  const serialNo =
                    (currentPage - 1) * recordsPerPage + globalIndex + 1;
                  return (
                    <tr
