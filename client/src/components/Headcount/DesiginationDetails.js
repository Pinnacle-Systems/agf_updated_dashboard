...
const DesiginationDetail = ({
 ...
 const downloadExcel = async () => {
    ...
    /* =======================
       1️⃣ TITLE ROW
    ======================= */
    worksheet.addRow(["Designation wise HeadCount Report"]);
    ...
    filteredData.forEach((row) => {
      worksheet.addRow([
        ...
        row.AGE !== null && row.AGE !== undefined
          ? Math.trunc(Number(row.AGE)) // trim decimals, no rounding
          : "",
      ]);
    });
    ...
    saveAs(new Blob([buffer]), "Designation wise HeadCount Report.xlsx");
  };
  return (
