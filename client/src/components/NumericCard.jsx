...
const NumericCard = ({ misData, selectedBuyer, checked }) => {
  ...
  const data = [
    {
      ...
      borderColor: "#1F588B",
      value: filteredTotalTurnOver.reduce(
        (acc, item) => acc + item.currentValue,
        0
      ),
      previousValue: filteredTotalTurnOver.reduce(
    ...
    },
    {
      ...
      borderColor: "#62AAA3",
      value: profit.reduce((acc, item) => acc + item.currentQty, 0),
      previousValue: profit.reduce((acc, item) => acc + item.comCode, 0),
      icon: staff,
    },
    {
      heading: "Labour Last Month Salary",
      borderColor: "#96A669",
      value: filterNewCus.reduce((acc, item) => acc + item.currentValue, 0),
      previousValue: filterNewCus.reduce(
        (acc, item) => acc + item.prevValue,
        0
      ),
      icon: money,
    },
    {
      ...
      borderColor: "#D49B37",
      value: filteredTopCus.reduce(
        (acc, item) => acc + item.currentValue,
        0
      ),
      previousValue: filteredTopCus.reduce(
        (acc, item) => acc + item.prevValue,
        0
      ),
      icon: staffs,
    ...
    {
      heading: checked ? "Employees' Esi Details" : "Employees' Pf Details",
      borderColor: "#F4A300",
    ...
    },
    {
      ...
      borderColor: "#F4A300",
      value:filterLoss1.reduce((acc, item) => acc + item.currentValue, 0),
      previousValue: filterLoss1.reduce((acc, item) => acc + item.prevValue, 0),
      icon: worker,
    },
  ];
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 p-4 bg-gradient-to-br from-[#1e293b] to-[#374151]">
      {data.map((val, i) => {
        const totalValue = val.value + val.previousValue;
        const malePercentage = calculatePercentage(
          val.previousValue,
          totalValue
        );
        ...
        return (
          ...
          >
            <div
              className={`absolute -top-2 -right-1 w-10 h-10 rounded-full flex items-center justify-center shadow-md`}
              style={{
                border: `2px solid ${val.borderColor}`,
                background:
                  color || "linear-gradient(to top right, #FFD700, #F4A300)",
              }}
            >
              <img
                src={
            ...
            <div className="text-center ">
              ...
              <div className="flex justify-between items-center">
                <p
                  className="text-2xl font-bold text-[#CA8A04] mb-1 mt-4"
                  style={{
                    color:
                      color ||
