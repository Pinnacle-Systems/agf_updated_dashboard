...
import { useGetOrderEntryColorWiseTableQuery } from "../../../../redux/service/OrderEntry";
...
/* ── Main ── */
const OrderEntryColorWisetable = ({
  finYear,
  compCode,
  finYr,
  closeTable,
  ...
  color: colorProp, // ← pre-filled from chart click
  allPoData,
  companyList
}) => {
  ...
  const [selectedColor, setSelectedColor] = useState(colorProp || "ALL"); // ← NEW
  ...
  const buyerCodes = useMemo(() => {
    if (buyerCodesProp?.length) return ["ALL", ...buyerCodesProp];
    if (!Array.isArray(allPoData)) return ["ALL"];
    return [
      "ALL",
      ...new Set(allPoData.map((d) => d.buyerCode).filter(Boolean)),
    ];
  }, [buyerCodesProp, allPoData]);

  // ── Color list derived from allPoData ────────────────────────────────────
  const colorList = useMemo(() => {
    ...
    return [
      ...
      ...new Set(
        allPoData
          .filter(
            (d) => selectedBuyer === "ALL" || d.buyerCode === selectedBuyer,
          )
          .map((d) => d.color)
          .filter(Boolean),
      ),
  ...
  // ── API fetch — passes color to backend ──────────────────────────────────
  const {
    data: ioRes,
    isLoading,
    isFetching,
  } = useGetOrderEntryColorWiseTableQuery(
    {
      params: {
        ...
        color: selectedColor, // ← wired to API
      },
    },
    { skip },
  );

  // ── NOTE: backend SELECT order is:
  //    po[0]=compCode, po[1]=typeName, po[2]=orderNo, po[3]=orderDate,
  //    po[4]=buyerCode, po[5]=buyerName, po[6]=bpoNo, po[7]=styleRefNo,
  //    po[8]=orderPackType, po[9]=color,               ← COLOR at index 9
  //    po[10]=orderQty, po[11]=excessQty, po[12]=amount
  // Make sure your RTK endpoint maps these correctly ↑
  ...
  const handleExport = async () => {
    ...
    const ws = wb.addWorksheet("Order Entry Color Wise");
    ...
    // Title row
    ws.insertRow(1, ["Order Entry Color Wise Quantity"]);
    ...
    addInsightsRowTurnOver({
      ...
      secondDynamicField: "Color",
      seconddynamicValue: selectedColor,
    });
    ...
    saveAs(
      ...
      `OrderEntry_ColorWise_${selectedColor}_${selectedBuyer}_${selectedYear}.xlsx`,
    );
  };
      ...
      <div className="bg-white w-[1360px] h-[630px] p-4 rounded-xl relative">
        ...
        <div className="flex justify-between items-center">
          <h2 className="font-bold uppercase text-sm">
            Order Entry Color Wise Quantity –{" "}
            ...
            <div className="bg-gray-300 rounded-lg shadow-2xl flex gap-x-2 gap-1 p-2 flex-wrap items-center">
              ...
              {/* <select
                ...
                onChange={(e) => {
                  ...
                  setSelectedColor("ALL"); // reset color on buyer change
                  resetPage();
                }}
              ...
              {/* Color — replaces old BPO No dropdown */}
              <select
                value={selectedColor}
                onChange={(e) => {
        ...
        >
          <table className="w-[1670px] border-collapse text-[11px] table-fixed">
...
export default OrderEntryColorWisetable;
