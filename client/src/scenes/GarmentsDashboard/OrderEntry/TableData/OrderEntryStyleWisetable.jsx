...
import { useGetOrderEntryStyleWiseTableQuery } from "../../../../redux/service/OrderEntry";
...
/* ── Main ── */
const OrderEntryStyleWisetable = ({
  finYear,
  compCode,
  finYr,
  closeTable,
  buyerCode,
  styleGroup: styleGroupProp, // ← NEW: pre-filled from chart click
  styleItem: styleItemProp, // ← NEW: pre-filled from chart click
  allPoData,
  companyList
}) => {
  ...
  // ── NEW: styleGroup / styleItem state ────────────────────────────────────
  ...
  // ── Derive dropdown options from allPoData ────────────────────────────────
    ...
    return [
      ...
      ...new Set(
        allPoData
          .filter(
            (d) =>
              (selectedBuyer === "ALL" || d.buyerCode === selectedBuyer) &&
              (selectedStyleGroup === "ALL" ||
                d.styleGroup === selectedStyleGroup),
          )
          .map((d) => d.styleItem)
          .filter(Boolean),
      ),
    ];
  }, [allPoData, selectedBuyer, selectedStyleGroup]);

  // ── API fetch — new endpoint with styleGroup + styleItem ─────────────────
  ...
  const handleExport = async () => {
    ...
    // Title row
    ws.insertRow(1, ["Order Entry Style Item Wise Quantity"]);
    ...
    addInsightsRowTurnOver({
      ...
      secondDynamicField: "Style Group",
      seconddynamicValue: selectedStyleGroup,
      thirdDynamicField: "Style Item Name",
      thirdDynamicValue: selectedStyleItem,
    });
    ...
    saveAs(
      ...
      `OrderEntry_StyleWise_${selectedStyleGroup}_${selectedStyleItem}_${selectedYear}.xlsx`,
    );
  };
          ...
          <div className="flex gap-2 items-center">
            <div className="bg-gray-300 rounded-lg shadow-2xl flex gap-x-2 gap-1 p-2 flex-wrap items-center">
              ...
              <select
                ...
                onChange={(e) => {
                  setSelectedStyleGroup(e.target.value);
                  setSelectedStyleItem("ALL");
                  resetPage();
                }}
...
export default OrderEntryStyleWisetable;
