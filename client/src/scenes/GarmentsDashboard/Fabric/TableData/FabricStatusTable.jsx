...
/* ── Status badge helper ── */
const StatusBadge = ({ inbal }) => {
  const isInHouse = Number(inbal || 0) <= 0;
  return (
    <span
      className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${isInHouse ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
        }`}
    >
      {isInHouse ? "In House" : "In Progress"}
    </span>
  );
};
...
const FabricStatusTable = ({
  ...
  const buyerList = useMemo(() => {
    const names = buyerNames?.length ? buyerNames : [];
  ...
  const handleExport = async () => {
    ...
    const ws = wb.addWorksheet("Fabric Status");
    ...
    filtered.forEach((r, i) => {
      ws.addRow({
        ...
        STATUS: Number(r.INBAL || 0) <= 0 ? "In House" : "In Progress",
      });
    });
    ...
    ws.eachRow((row, rn) => {
      ...
      row.eachCell((cell, cn) => {
        ...
        if (isQty) cell.numFmt = "#,##0.000"; // Color Fabric Inhouse Bal Qty (INBAL) red if > 0
        if (key === "INBAL") {
        ...
        // ← ADD THIS: Status cell color
        if (key === "STATUS") {
          const isInHouse = cell.value === "In House";
          cell.font = {
            bold: true,
            color: { argb: isInHouse ? "FF15803D" : "FFDC2626" }, // green-700 : red-600
          };
          cell.fill = {
            type: "pattern",
            pattern: "solid",
            fgColor: { argb: isInHouse ? "FFDCFCE7" : "FFFEE2E2" }, // green-100 : red-100
          };
          cell.alignment = { horizontal: "center", vertical: "middle" };
        }
      });
    ...
    saveAs(
      ...
      `FabricStatus_${selectedBuyer}_${selectedStatus}_${selectedYear}.xlsx`,
    );
  };
...
export default FabricStatusTable;
