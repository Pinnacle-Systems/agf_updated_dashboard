...
import { useGetAccessoryProcessDataQuery, useGetProcessDataQuery } from "../../../redux/AgfServices/ProcessDetails";
import AccessoryProcessDetailsTable from "./TableData/AccessoryProcessDetailsTable";

const AccessoryProcessDetails = ({
  filterBuyer,
  selectedYear,
  selectMonths,
  finYr,
  ...
  const options = useMemo(() => ({
    ...
    series: [
      {
        name: "Accessory Process Details",
  ...
  if (!chartData || chartData.length === 0) {
    return (
      <Card sx={{ borderRadius: 3, boxShadow: 4, width: "100%", ml: 1 }}>
        <CardHeader
          title="Accessory Process Details (OutSide Suppliers)"
...
export default AccessoryProcessDetails;