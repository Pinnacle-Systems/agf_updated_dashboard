...
const Form = ({ companyName, finYear, finYr, filterBuyerList }) => {
  ...
  const { data: response, isLoading } = useGetMisDashboardErpMonthWiseQuery({
  ...
  /* ---------------- Parent ---------------- */
  const categories = useMemo(() => chartData.map((i) => i.month), [chartData]);

  const seriesData = useMemo(() => chartData.map((i) => i.value), [chartData]);
