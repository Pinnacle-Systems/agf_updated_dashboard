...
const SupplierEfficiencyTable = ({
  ...
  const {
    ...
    isLoading: l0,
    isFetching: f0,
  } = useGetSupplierEfficiencyGeneralTableQuery(qParams, { skip });
  const {
    data: greyYarnRes,
    isLoading: l1,
    isFetching: f1,
  } = useGetSupplierEfficiencyGreyYarnTableQuery(qParams, { skip });
  const {
    data: dyedYarnRes,
    isLoading: l2,
    isFetching: f2,
  } = useGetSupplierEfficiencyDyedYarnTableQuery(qParams, { skip });
  const {
    data: greyFabricRes,
    isLoading: l3,
    isFetching: f3,
  } = useGetSupplierEfficiencyGreyFabricTableQuery(qParams, { skip });
  const {
    data: dyedFabricRes,
    isLoading: l4,
    isFetching: f4,
  } = useGetSupplierEfficiencyDyedFabricTableQuery(qParams, { skip });
  const {
    data: accessoryRes,
    isLoading: l5,
    isFetching: f5,
  } = useGetSupplierEfficiencyAccessoryTableQuery(qParams, { skip });
  ...
  const buildExcel = async (data, label, fileName) => {
    ...
    const ws = wb.addWorksheet("Supplier Efficiency Report");
    ...
    const columns = [
      ...
      { header: "Earlier Days", key: "days", width: 14 },
    ];
    ws.columns = columns;

    ws.insertRow(1, [`Supplier Efficiency Report — ${label}`]);
    ws.mergeCells(`A1:G1`);
    ...
    ws.eachRow((row, rn) => {
      ...
      row.eachCell((cell, cn) => {
        ...
        // Red bold font for Earlier Days column
        if (key === "days") {
  ...
  const downloadSelected =
    localPoType === "General"
      ? () => buildExcel(fd0, "General", "Supplier_Efficiency_General.xlsx")
      : selectedOrderType === "GREY YARN"
        ? () =>
            buildExcel(fd1, "Grey Yarn", "Supplier_Efficiency_GreyYarn.xlsx")
        : selectedOrderType === "DYED YARN"
          ? () =>
              buildExcel(fd2, "Dyed Yarn", "Supplier_Efficiency_DyedYarn.xlsx")
          : selectedOrderType === "GREY FABRIC"
            ? () =>
                buildExcel(
                  fd3,
                  "Grey Fabric",
                  "Supplier_Efficiency_GreyFabric.xlsx",
                )
            : selectedOrderType === "DYED FABRIC"
              ? () =>
                  buildExcel(
                    fd4,
                    "Dyed Fabric",
                    "Supplier_Efficiency_DyedFabric.xlsx",
                  )
              : selectedOrderType === "ACCESSORY"
                ? () =>
                    buildExcel(
                      fd5,
                      "Accessory",
                      "Supplier_Efficiency_Accessory.xlsx",
                    )
                : null;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-[9999] flex justify-center items-center">
      <div className="bg-white w-[1470px] h-[630px] p-4 rounded-xl relative">
        ...
        <p className="text-xs font-semibold text-gray-600">
          Total Earlier Days:{" "}
          <span className="text-green-600">{displayTotal}</span>
        </p>
                  ...
                  {l0 || f0 ? (
                    ...
                    cr0.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="border p-1 pr-2 text-right font-semibold text-green-600">
                          {row.days}
                        </td>
                      </tr>
                    ))
...
export default SupplierEfficiencyTable;
