...
const TopTenItemstable = ({
  ...
  const { data: generalRes,    isLoading: l0, isFetching: f0 } = useGetTopTenItemGeneralTableQuery(qParams,    { skip });
  const { data: greyYarnRes,   isLoading: l1, isFetching: f1 } = useGetTopTenItemGreyYarnTableQuery(qParams,   { skip });
  const { data: dyedYarnRes,   isLoading: l2, isFetching: f2 } = useGetTopTenItemDyedYarnTableQuery(qParams,   { skip });
  const { data: greyFabricRes, isLoading: l3, isFetching: f3 } = useGetTopTenItemGreyFabricTableQuery(qParams, { skip });
  const { data: dyedFabricRes, isLoading: l4, isFetching: f4 } = useGetTopTenItemDyedFabricTableQuery(qParams, { skip });
  const { data: accessoryRes,  isLoading: l5, isFetching: f5 } = useGetTopTenItemAccessoryTableQuery(qParams,  { skip });
          ...
          <div className="flex gap-2 items-center">
            <div className="bg-gray-300 rounded-lg shadow-2xl flex gap-x-2 gap-1 p-2 flex-wrap">
              ...
              <select value={selectedItem}
                onChange={(e) => { setSelectedItem(e.target.value); resetAllPages(); }}
                  ...
                  {l0 || f0 ? <LoadingRow cols={10} /> : cr0.length === 0 ? <EmptyRow cols={10} /> :
                    cr0.map((row, i) => (
                      <tr key={i} className="text-gray-800 bg-white even:bg-gray-100">
                        ...
                        <td className="border p-1 pl-2 text-left">{row.item}</td>
                  ...
                  {l5 || f5 ? <LoadingRow cols={13} /> : cr5.length === 0 ? <EmptyRow cols={13} /> :
                    cr5.map((row, i) => (
                      <tr key={i} className="text-gray-800 bg-white even:bg-gray-100">
                        ...
                        <td className="border p-1 pl-2 text-left">{row.accessItemDesc}</td>
                        <td className="border p-1 pl-2 text-left">{row.docId}</td>
                        <td className="border p-1 pl-2 text-left">{fmtDate(row.docDate)}</td>
                        <td className="border p-1 pl-2 text-left">{row.supplier}</td>
                        <td className="border p-1 pr-2 text-left">{row.orderNo}</td>
