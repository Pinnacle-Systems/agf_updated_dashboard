...
import { useGetKnittingStoreGRNTableQuery } from "../../../../redux/AgfServices/GRNservices";
import ExcelJS from "exceljs";

const KnittingStoreGRNTable = ({ open, onClose, companyName, selectedYear }) => {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [searchTerm, setSearchTerm] = useState("");

  const { data: response, isLoading } = useGetKnittingStoreGRNTableQuery(
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Knitting Store GRN");

    worksheet.addRow(["Knitting Store Purchase GRN Register"]);
    ...
    link.href = url;
    link.download = `${companyName}_Knitting_Store_GRN_${selectedYear}.xlsx`;
    link.click();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff" }}>
        <Typography variant="h6">Knitting Store Purchase GRN Log ({companyName} — {selectedYear})</Typography>
          ...
          <>
            <Box display="flex" justifyContent="space-between" mb={2} gap={2}>
              <TextField
                variant="outlined"
                size="small"
                label="Search Knitting Store GRN..."
                value={searchTerm}
...
export default KnittingStoreGRNTable;
