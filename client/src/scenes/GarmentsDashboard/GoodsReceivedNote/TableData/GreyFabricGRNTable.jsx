...
import { useGetGreyFabricGRNTableQuery } from "../../../../redux/AgfServices/GRNservices";
import ExcelJS from "exceljs";

const GreyFabricGRNTable = ({ open, onClose, companyName, selectedYear }) => {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [searchTerm, setSearchTerm] = useState("");

  const { data: response, isLoading } = useGetGreyFabricGRNTableQuery(
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Grey Fabric GRN");

    worksheet.addRow(["Grey Fabric GRN Register"]);
    ...
    link.href = url;
    link.download = `${companyName}_Grey_Fabric_GRN_${selectedYear}.xlsx`;
    link.click();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff" }}>
        <Typography variant="h6">Grey Fabric GRN Log ({companyName} — {selectedYear})</Typography>
...
export default GreyFabricGRNTable;
