...
const EmbroideryAccessoryTable = ({ open, onClose, companyName, selectedYear }) => {
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Embroidery Inward");

    worksheet.addRow(["Embroidery Accessory Purchase Inward Register"]);
    ...
    const headers = ["S.No", "Date", "Inward No", "PO No", "Supplier", "Item Name", "Qty", "UOM", "Rate", "Amount"];
    ...
    link.href = url;
    link.download = `${companyName}_Embroidery_Inward_${selectedYear}.xlsx`;
    link.click();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff" }}>
        <Typography variant="h6">Embroidery Accessory Inward Log ({companyName} — {selectedYear})</Typography>
          ...
          <>
            <Box display="flex" justifyContent="space-between" mb={2} gap={2}>
              <TextField
                variant="outlined"
                size="small"
                label="Search Embroidery Inward..."
                value={searchTerm}
            ...
            <TableContainer component={Paper} sx={{ maxHeight: 500 }}>
              <Table stickyHeader>
                <TableHead>
                  <TableRow>
                    {["S.No", "Date", "Inward No", "PO No", "Supplier", "Item Name", "Qty", "UOM", "Rate", "Amount"].map((header) => (
...
export default EmbroideryAccessoryTable;
