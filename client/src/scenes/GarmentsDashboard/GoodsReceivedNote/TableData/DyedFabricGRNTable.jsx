...
import { useGetDyedFabricGRNTableQuery } from "../../../../redux/AgfServices/GRNservices";
import ExcelJS from "exceljs";

const DyedFabricGRNTable = ({ open, onClose, companyName, selectedYear }) => {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [searchTerm, setSearchTerm] = useState("");

  const { data: response, isLoading } = useGetDyedFabricGRNTableQuery(
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Dyed Fabric GRN");

    worksheet.addRow(["Dyed Fabric GRN Register"]);
    ...
    link.href = url;
    link.download = `${companyName}_Dyed_Fabric_GRN_${selectedYear}.xlsx`;
    link.click();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff" }}>
        <Typography variant="h6">Dyed Fabric GRN Log ({companyName} — {selectedYear})</Typography>
                ...
                <TableBody>
                  {filteredData.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, i) => (
                    <TableRow key={i} hover>
                      ...
                      <TableCell>{row.docDate ? new Date(row.docDate).toLocaleDateString("en-IN") : "-"}
                      </TableCell>
...
export default DyedFabricGRNTable;
