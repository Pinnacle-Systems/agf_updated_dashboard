...
import { useGetCuttingPrintingGRNTableQuery } from "../../../../redux/AgfServices/GRNservices";
import ExcelJS from "exceljs";

const CuttingPrintingGRNTable = ({ open, onClose, companyName, selectedYear }) => {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [searchTerm, setSearchTerm] = useState("");

  const { data: response, isLoading } = useGetCuttingPrintingGRNTableQuery(
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Cutting Printing GRN");

    worksheet.addRow(["Cutting / Printing Store GRN Register"]);
    ...
    const headers = ["S.No", "Date", "GRN No", "Supplier", "Item Group", "Item Name", "Qty", "UOM", "Rate", "Amount"];
    ...
    totalsRow.getCell(7).numFmt = "#,##0.000";
    ...
    link.href = url;
    link.download = `${companyName}_Cutting_Printing_GRN_${selectedYear}.xlsx`;
    link.click();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff" }}>
        <Typography variant="h6">Cutting / Printing Store GRN Log ({companyName} — {selectedYear})</Typography>
          ...
          <>
            <Box display="flex" justifyContent="space-between" mb={2} gap={2}>
              <TextField
                variant="outlined"
                size="small"
                label="Search Cutting/Printing GRN..."
                value={searchTerm}
            ...
            <TableContainer component={Paper} sx={{ maxHeight: 500 }}>
              <Table stickyHeader>
                <TableHead>
                  <TableRow>
                    {["S.No", "Date", "GRN No", "Supplier", "Item Group", "Item Name", "Qty", "UOM", "Rate", "Amount"].map((header) => (
...
export default CuttingPrintingGRNTable;
