...
const AccessoryGRNDetailsTable = ({ open, onClose, companyName, selectedYear, subReportType }) => {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [searchTerm, setSearchTerm] = useState("");

  const accQuery = useGetAccessoryGRNTableQuery(
    { params: { selectedYear, companyName } },
    { skip: subReportType !== "Accessory" || !open }
  );

  const embQuery = useGetEmbroideryAccessoryInwardTableQuery(
    { params: { selectedYear, companyName } },
    { skip: subReportType !== "Embroidery" || !open }
  );

  const activeQuery = subReportType === "Accessory" ? accQuery : embQuery;
  ...
  const exportToExcel = async () => {
    ...
    worksheet.addRow([`${subReportType} Goods Inward Register`]);
    ...
    link.href = url;
    link.download = `${companyName}_${subReportType}_Accessory_Inward_${selectedYear}.xlsx`;
    link.click();
  };
    ...
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff", py: 2 }}>
        <Typography variant="h6" fontWeight="600">
          {subReportType} Inward Log ({companyName} — {selectedYear})
        </Typography>
          ...
          <>
            <Box display="flex" justifyContent="space-between" mb={2} gap={2} flexWrap="wrap">
              <TextField
                variant="outlined"
                size="small"
                label="Search GRN, Supplier or Trim Name..."
                value={searchTerm}
...
export default AccessoryGRNDetailsTable;
