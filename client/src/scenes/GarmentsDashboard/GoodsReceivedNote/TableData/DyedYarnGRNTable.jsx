...
import { useGetDyedYarnGRNTableQuery } from "../../../../redux/AgfServices/GRNservices";
import ExcelJS from "exceljs";

const DyedYarnGRNTable = ({ open, onClose, companyName, selectedYear }) => {
  const [page, setPage] = useState(0);
  const [rowsPerPage, setRowsPerPage] = useState(25);
  const [searchTerm, setSearchTerm] = useState("");

  const { data: response, isLoading } = useGetDyedYarnGRNTableQuery(
  ...
  const exportToExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Dyed Yarn GRN");

    worksheet.addRow(["Dyed Yarn GRN Register"]);
    ...
    const headers = ["S.No", "Date", "GRN No", "PO No", "Supplier", "Yarn Name", "Color", "Qty (Kg)", "UOM", "Rate", "Amount"];
    ...
    link.href = url;
    link.download = `${companyName}_Dyed_Yarn_GRN_${selectedYear}.xlsx`;
    link.click();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xl" fullWidth>
      <DialogTitle sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", bgcolor: "#1e3a8a", color: "#fff" }}>
        <Typography variant="h6">Dyed Yarn GRN Log ({companyName} — {selectedYear})</Typography>
          ...
          <>
            ...
            <TableContainer component={Paper} sx={{ maxHeight: 500 }}>
              <Table stickyHeader>
                <TableHead>
                  <TableRow>
                    {["S.No", "Date", "GRN No", "PO No", "Supplier", "Yarn Name", "Color", "Qty (Kg)", "UOM", "Rate", "Amount"].map((header) => (
...
export default DyedYarnGRNTable;
