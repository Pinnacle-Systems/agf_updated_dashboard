...
import { useGetMisDashboardErpCountryWiseQuery } from
...
const CountryWiseTable = ({
    ...
    const [selectedCountry, setSelectedCountry] = useState(countryName || "ALL");
    ...
    const customerOptions = useMemo(() => {
        const unique = [...new Set(rawData.map(r => r.countryName))];
    ...
    const filteredData = useMemo(() => {
        return rawData.filter((row) => {
            ...
            if (selectedCountry !== "ALL" && row.countryName !== selectedCountry) {
                return false;
            }

            // 🔹 Search filter (countryName search)
            if (search.countryName) {
                const rowCustomer = row.countryName?.toLowerCase() || "";
                if (!rowCustomer.includes(search.countryName.toLowerCase())) {
                    return false;
                }
            }
    ...
    }, [rawData, selectedCountry, search, netpayRange]);


    useEffect(() => {
        setSelectedCountry(countryName || "ALL");
        setCurrentPage(1);
    }, [countryName]);
    useEffect(() => {
    ...
    const downloadExcel = async () => {
        ...
        worksheet.columns = [
            ...
            { header: "Country", key: "countryName", width: 40 },
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            row.getCell("countryName").alignment = { horizontal: "left", vertical: "middle", indent: 1 };
            ...
            <div className="bg-white w-[1300px] h-[620px] p-4 rounded-xl relative">
                ...
                <div className="flex justify-between items-center">
                    <h2 className="font-bold uppercase">
                        Country Wise Turnover - <span className="text-blue-600 ">{localCompany || ""}</span>
                    </h2>
                        ...
                        <table className="w-full border-collapse border border-gray-300 text-[11px] table-fixed">
                            <thead className="bg-gray-100 text-gray-800 sticky top-0 tracking-wider">
                                <tr>
                                    ...
                                    <th className="border p-1 text-center w-16">Country</th>
                                ...
                                {currentRecords.slice(0, 17).map((row, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="border p-1 text-left">{row.countryName}</td>
