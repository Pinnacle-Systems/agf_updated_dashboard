...
const CompESI = ({
  ...
  let excelTitle = "ESI  Employer Vs Employee   Contribution Report"
  ...
  const groupdata = filteredData?.reduce((acc, emp) => {
    ...
    if (!acc[code]) {
      acc[code] = { employer: 0, esi: 0, total: 0 };
    }

    acc[code].employer += emp.EMPLOYER_CON || 0;
    acc[code].esi += emp.ESI || 0;
    acc[code].total = acc[code].employer + acc[code].esi;
  ...
  const option = {
      ...
      events: {
        render() {
          ...
          if (!customLabel) {
            ...
            if (chart1) {
              customLabel.attr({
                text: `Total<br/><strong>${chart1?.total.toLocaleString(
                  "en-IN"
                )}</strong>`,
              });
            }else{
    ...
    tooltip: {
      formatter() {
        return `${
          this.point.name
        } share </b><br/>ESI: ${this.point.y.toLocaleString("en-IN")}`;
      },
    },
    legend: {
      enabled: false,
