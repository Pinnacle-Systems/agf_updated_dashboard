...
const EmpType1= ({ companyName, selectedState, salary,selectmonths,selectedYear1 }) => {
  ...
  const Chartdata = Object.entries(totalsByComp).map(([emptype, netpay], index) => ({
    Emptype: emptype,
    Netpay: netpay,
   ...
   <Card
      ...
      <CardContent>
        <Box sx={{ display: '', alignItems: 'center', gap:1 }}>
          ...
          <Box sx={{ width: 200, height: 200,ml:4 }}>
            <ResponsiveContainer>
              <PieChart>
                <Pie
                  data={Chartdata}
            ...
            {Chartdata.map((item, index) => (
              <Box key={index} sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                ...
                <Typography variant="body2" sx={{fontSize:"11px"}}>
                  <strong>{item.Netpay.toLocaleString("en-IN")}</strong> — {item.Emptype}
                </Typography>
              </Box>
            ))}
          </Box>
