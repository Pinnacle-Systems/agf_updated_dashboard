...
const CompanyAllocation = ({
  ...
  const [createUser] = useAddCompanyMutation();
  ...
  console.log(allData, "SingleData ");

  // console.log(multiSelectOption(compCode ? compCode?.data : [], "com", "com"));
  ...
  const handleSubmit = async () => {
    ...
    if (edit) {
      if (!username ||Object.keys(COMPCODE).length === 0) {
        Swal.fire({
      ...
      !username ||Object.keys(COMPCODE).length === 0
    ) {
      Swal.fire({
        title: "please fill the required fields",
        icon: "warning",
    ...
    <>
      ...
      <div className="space-y-3 h-full py-3">
        <div className="border border-slate-200 p-2 bg-white rounded-md shadow-sm col-span-1">
          <h2 className="font-medium text-slate-700 mb-4">User Details</h2>
